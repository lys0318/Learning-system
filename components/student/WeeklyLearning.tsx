...
export default function WeeklyLearning({
  ...
  const totalItems = materials.length + quizzes.length;
  const isCompleted = progress === 100;

  function recalcLocalProgress(newCompletedIds: Set<string>) {
    const done = newCompletedIds.size + completedQuizIds.size;
  ...
  const weekItemCount = weekMaterials.length + weekQuizzes.length;
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        {Array.from({ length: totalWeeks }, (_, i) => i + 1).map((w) => {
          ...
          const wTotal = wMats.length + wQuizzes.length;
          const wDone = wMatDone + wQuizDone;
      ...
      <div className="bg-[#16213e] rounded-xl border border-gray-700/50 p-5 space-y-5">
        ...
        {weekItemCount === 0 && (
          <div className="text-center py-8 text-gray-500 text-sm">
            {activeWeek}주차에 등록된 자료나 퀴즈가 없습니다.
          </div>
        )}
      </div>
    </div>
