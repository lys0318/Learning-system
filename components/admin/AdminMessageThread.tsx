...
export default function AdminMessageThread({ teacherId, teacherName, adminId, initialMessages }: Props) {
          ...
          return (
            <div key={msg.id}>
              ...
              <div className={`flex items-end gap-2 ${isMe ? "flex-row-reverse" : "flex-row"}`}>
                {!isMe && (
                  <div className="w-7 h-7 rounded-full bg-indigo-900/60 flex items-center justify-center text-xs font-medium text-indigo-300 shrink-0 mb-1">
      ...
      <div className="shrink-0 px-4 py-3 border-t border-gray-200 dark:border-gray-700/50 bg-white dark:bg-[#0d1224]">
        <div className="flex items-end gap-2">
          <textarea
            ...
            placeholder={`${teacherName} 강사님에게 메시지 보내기… (Enter 전송, Shift+Enter 줄바꿈)`}
            rows={1}
            className="flex-1 px-3.5 py-2.5 rounded-xl bg-gray-50 dark:bg-gray-800/60 border border-gray-200 dark:border-gray-700 text-sm text-gray-900 dark:text-gray-200 placeholder-gray-400 dark:placeholder-gray-600 resize-none focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500/50 max-h-32 overflow-y-auto"
            style={{ height: "auto" }}
            onInput={(e) => {
          ...
          <button
            ...
            className="shrink-0 w-9 h-9 rounded-xl bg-purple-600 hover:bg-purple-700 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center transition-colors"
          >
