  ...
  return (
    <main className="max-w-3xl mx-auto px-8 py-8">
      ...
      <nav className="flex items-center gap-2 text-sm text-gray-400 mb-6">
        <Link href="/teacher/quizzes" className="hover:text-white transition-colors">퀴즈 관리</Link>
        <span>/</span>
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-400 text-sm mt-1">총 {questions.length}문항 · 정답 포함 미리보기</p>
      </div>

      {questions.length === 0 ? (
        <div className="bg-[#16213e] rounded-xl border border-yellow-500/30 p-10 text-center">
          <p className="text-yellow-400 text-sm mb-4">이 퀴즈에는 문항 데이터가 없습니다. 삭제 후 다시 생성해주세요.</p>
          <Link
        ...
        <div className="space-y-5">
          {questions.map((q, idx) => (
            <div key={idx} className="bg-[#16213e] rounded-xl border border-gray-700/50 p-6">
                    ...
                    >
                      <span
                        className={`w-4 h-4 shrink-0 rounded-full border flex items-center justify-center text-xs ${
                          isAnswer ? "border-green-400 bg-green-400 text-black" : "border-gray-600"
                        }`}
                      >
                        {isAnswer ? "✓" : ""}
                      </span>
                      {opt}
                      {isAnswer && (
                        <span className="ml-auto text-xs text-green-400 font-semibold">정답</span>
                      )}
                    </li>
                  );
                })}
