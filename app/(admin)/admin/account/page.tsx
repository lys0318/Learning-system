...
export default async function AdminAccountPage() {
  ...
  const initial = (profile.full_name ?? "AD").slice(0, 2);
  ...
  return (
    <main className="px-6 py-6 max-w-lg space-y-5">
      ...
      <div className="bg-white dark:bg-[#16213e] rounded-xl border border-gray-200 dark:border-gray-700/50 p-5 flex items-center gap-4">
        <div className="w-14 h-14 rounded-full bg-purple-900 flex items-center justify-center text-lg font-bold text-purple-300 shrink-0">
          {initial}
        </div>
        <div>
          <p className="text-base font-semibold">{profile.full_name ?? "-"}</p>
          <p className="text-sm text-gray-500 dark:text-gray-400">{user.email}</p>
          <span className="mt-1 inline-block text-xs px-2 py-0.5 rounded-full bg-purple-600/20 text-purple-300 border border-purple-500/30">
            교육운영자
          </span>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-[#16213e] rounded-xl border border-gray-200 dark:border-gray-700/50 p-5">
        ...
        <p className="text-xs text-gray-500 dark:text-gray-400 mb-4">계정을 삭제하면 모든 데이터가 영구 삭제됩니다.</p>
        <DeleteAccountButton />
      </div>
    </main>
  );
