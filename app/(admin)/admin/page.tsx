    ...
    <main className="px-6 py-6 space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        <div className="bg-[#16213e] rounded-xl border border-gray-700/50 p-4">
          ...
          <p className="text-gray-500 text-xs mt-1">강사 {teacherCount ?? 0}명 포함</p>
        </div>
        <div className="bg-[#16213e] rounded-xl border border-gray-700/50 p-4">
        ...
        <div className="bg-[#16213e] rounded-xl border border-gray-700/50 p-4">
          <p className="text-gray-400 text-xs mb-2">전체 수료율</p>
      ...
      <div className="grid lg:grid-cols-5 gap-6">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                <h2 className="text-sm font-semibold text-gray-300">이탈 위험 수강생</h2>
            ...
            <div className="bg-[#16213e] rounded-xl border border-red-500/20 overflow-hidden">
              {atRisk.length > 0 ? (
                    ...
                    {atRisk.map((e, i) => {
                      ...
                      return (
                        <tr key={i} className="border-b border-gray-700/30 last:border-0">
                          <td className="px-4 py-2.5 font-medium text-white">{e.profiles?.full_name ?? "-"}</td>
                          <td className="px-4 py-2.5 text-gray-400 max-w-[120px] truncate">{e.courses?.title ?? "-"}</td>
                          <td className="px-4 py-2.5 text-right">
                            <span className="text-red-400 font-semibold">{e.progress}%</span>
                          </td>
                          <td className="px-4 py-2.5 text-right text-gray-500">{daysSince}일 전</td>
                        </tr>
                      );
                    })}
                  </tbody>
                ...
                <p className="text-gray-500 text-xs text-center py-6">이탈 위험 수강생이 없습니다 🎉</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          {/* 수료 현황 요약 */}
          <div>
            <h2 className="text-sm font-semibold text-gray-300 mb-3">수료 현황</h2>
            <div className="bg-[#16213e] rounded-xl border border-gray-700/50 p-4 space-y-3">
          ...
          <div>
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <p className="text-xs text-gray-400 mt-0.5">강사별 평점 및 수료율 분석</p>
              </Link>
            </div>
          </div>
        </div>
